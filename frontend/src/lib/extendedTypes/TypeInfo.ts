...
export class TypeInfo {
    ...
    public static getTypeId(object: any): string {
        return this.create(object).getTypeId()
    }
    ...
    public addPropertyMetadata<T>(property: string, name: string, value: Partial<T>): TypeInfo {
        const typeInfo = this.getObjectType()
        if (!typeInfo.propertyMeta[property]) {
            typeInfo.propertyMeta[property] = {}
        }
        typeInfo.propertyMeta[property][name] = { ...typeInfo.propertyMeta[property][name], ...value }
        return this
    }
